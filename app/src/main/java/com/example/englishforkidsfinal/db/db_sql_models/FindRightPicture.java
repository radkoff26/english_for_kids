...
import com.example.englishforkidsfinal.models.db_models.FindRightPictureModel;
...
public class FindRightPicture {

    private LearnedWordsDataBase thisDB;

    public FindRightPicture(Context ctx) {
        thisDB = new LearnedWordsDataBase(ctx);
    }

    // Method to get all words in Database
    public List<FindRightPictureModel> getWords() {
        List<FindRightPictureModel> words = new ArrayList<>();
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                FindRightPictureModel word_to_add = new FindRightPictureModel(
                        cursor.getInt(id),
